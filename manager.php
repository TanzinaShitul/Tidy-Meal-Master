...
$mobile = array();
$st = array();
$sql = "SELECT DISTINCT account.ID as ID, Email, Mobile, ST, Name FROM account where account.ManagerID = $userID";
$result = $conn->query($sql);
...
<body id="myPage" data-spy="scroll" data-target=".navbar" data-offset="50">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="myNavbar">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li><a href="loout.php" data-toggle="tooltip" data-placement="bottom" title="Log Out"><span
    ...
    <div class="cls1"> <!-- This div Containing Whole thing-->
        <div id="profile" class="container-fluid text-center cls2"><!-- This Is Profile Div-->
            ...
            ?>
            <div class="col-sm-12">
                <img id="propic" src="<?php echo $result["Pro"]; ?>" class="img-circle" alt="User" width="220"
                    height="220">
            </div>
            <h1>
            ...
            <div class="container text-center">
                    ...
                    <table class="container-fluid clstbl">
                        ...
                        <tbody>
                            ...
                            for ($x = 0; $x < $arrlength; $x++) {
                                ...
                                $sm = (0 + $row["total"]);
                                echo '<td>' . $sm . '</td>';
                                if ($AllMemberID[$x] == 1 && $_SESSION["STUS"] == "manager") {
                                    $AllMoney = $sm;
                                }
                                echo '</tr>';
                            }
                            ...
                            if ($userID != "") {
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $MEAL[] = $row["Amount"];
                                        $AllMEal += $row[$tm3];
                                        $AllMealCost += $row["Amount"] * $row[$tm3];
                                    }
                                }

                                $tm = "member" . $userID;
                                $sql = "SELECT COUNT(*) AS total FROM {$tm} WHERE Type!='join'";
                                $member = $conn->query($sql)->fetch_assoc()['total'];
                                $member = max(1, $member);

                                $tm = "ext" . $userID;
                                $ExtraCost = round($conn->query("SELECT SUM(Amount) as total FROM {$tm}")->fetch_assoc()['total'] / $member, 2);

                                $total = $AllMealCost + $FixedCost + $ExtraCost;
                                $Due = max(0, $total - $AllMoney);
                                $Extra = max(0, $AllMoney - $total);
                            }
                ...
                <div class="cls5 slideanim">
                    <table class="container-fluid clstbl">
                        ...
                        <tbody>
                            <?php
                            foreach ($AllMemberID as $i => $memberID) {
                                $sql = "SELECT mealdate{$userID}.Amount AS Amount, mealdate{$userID}.Mdate AS Dat, meal{$userID}.A{$memberID} FROM meal{$userID} INNER JOIN mealdate{$userID} ON mealdate{$userID}.ID=meal{$userID}.ID";
                                $result = $conn->query($sql);

                                $AllMembersMealCost[$memberID] = 0;
                                $totalMeals = 0;

                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        $AllMembersMealCost[$memberID] += $row["Amount"] * $row["A{$memberID}"];
                                        $totalMeals += $row["A{$memberID}"];
                                    }
                                }

                                echo '<tr>';
                                echo '<td>' . $AllMemberName[$i] . '</td>';

                                $tm = 'bal' . $AllMemberID[$i];
                                $sql = "SELECT SUM(Amount) as total from {$tm}";
                                $result = $conn->query($sql);
                                $sm = (0 + $result->fetch_assoc()["total"]);

                                echo '<td>' . $sm . '</td>';
                                echo '<td>' . $totalMeals . '</td>';

                                $total = $AllMembersMealCost[$memberID] + $FixedCost + $ExtraCost;
                                $due = max(0, $total - $sm);
                                $balance = max(0, $sm - $total);
                ...
                <br>
                <br>
                <!-- all member info and delete -->
                <h2>#All Members Info.</h2>
                <div class="cls5 slideanim">
                    <table class="container-fluid clstbl">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            foreach ($idArray as $id => $value) {
                                echo '<tr>';
                                echo '<td>' . $AllMemberName[$id] . '</td>';
                                echo '<td>' . $memberEmail[$id] . '</td>';
                                echo '<td>' . $mobile[$id] . '</td>';
                                echo '<td>' . $st[$id] . '</td>';

                                if ($AllMemberID[$id] != $userID) {
                                    echo "<td><a class='delete-link' href='?deleteEmail=" . urlencode($memberEmail[$id]) . "&deleteID=" . $value . "'>Delete</a></td>";
                                } else {
                                    echo "<td></td>";
                                }
                                echo '</tr>';
                            ...
                            if (isset($_GET['deleteEmail']) && isset($_GET['deleteID'])) {
                                // Get the email to delete from the URL parameter
                                $deleteEmail = $_GET['deleteEmail'];
                                $deleteId = $_GET['deleteID'];



                                // // Delete user from both tables using a single query with a JOIN
                                $deleteQuery = "DELETE member{$userID}, account FROM member{$userID} INNER JOIN account ON member{$userID}.ID = account.ID WHERE member{$userID}.ID = $deleteId AND account.Email = '$deleteEmail'";                            
                                if ($conn->query($deleteQuery) === true) {
                                    $dropTableChat = "DROP TABLE chat{$deleteId}";
                                    $dropTablebal = "DROP TABLE bal{$deleteId}";

                                    if ($conn->query($dropTableChat) === true && $conn->query($dropTablebal) === true) {
                                        echo '<script>
        ...
        <footer class="text-center">
            ...
            <p>Developed By : "Tanzina & Siti", Dept. of CSE LU,Sylhet.</p>
        </footer>
    ...
    <div class="modal fade modal-fullscreen  " id="ClogGroup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" style="font-size: 50px;font-weight: lighter;"
                ...
                <div class="modal-body container-fluid">
                    ...
                    <div class="container" style="color:#FB667A;">
                        <div class="mdl_form text-center">
                            ...
                            <p>This operation means that you are wanting to closing your current meal management
                                group.
                                If You Perform This Operation all information will be lost. Please check all the
                                information and let your member awar about this action. If you already conncern
                                about
                                this then continue.</p>
                        </div>
                        <br>
                        <br>
                        <br>
                        ...
                        <div class="col-sm-12">
                            <div class="col-sm-12 text-center">
                                <button id="CloseGroup" type="button" class="btn btn-danger">Close Your Group. <span
    ...
    <div class="modal fade modal-fullscreen  " id="AddNotice" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="container">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" style="resize:none;" rows="10" id="ANtmsg"
                                    maxlength="1490" placeholder="Type:"></textarea>
                            </div>
                        </div>
                        <div class="col-sm-12">
                            <button id="ANTbtn" type="button" class="btn btn-primary">OK <span
    ...
    <div class="modal fade modal-fullscreen  " id="Common_Meal" tabindex="-1" role="dialog"
                        ...
                        <div class="row mdl_edit text-center">
                            ...
                            <div class="col-sm-6 form-group" style="text-align: left; color:#FB667A;">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="lastcmdate" readonly
                                        value="<?php echo $LastMealDate; ?>">
                                </div>
                            </div>
                            <div class="col-sm-6 form-group" style="text-align: left; color:#FB667A;">
                                <div class="form-group">
                                    <label for="Lastcmcost">Last Meal Rate:</label>
                                    <input type="text" class="form-control" id="Lastcmcost" readonly
                                        value="<?php echo $LastMealAmount; ?>">
                                </div>
                            </div>
                            <div class="col-sm-6 form-group" style="text-align: left; color:#FB667A;">
                                <div class="form-group">
                            ...
                            <div class="col-sm-6 form-group" style="text-align: left; color:#FB667A;">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="cmcost"
                                        onkeypress='return event.charCode >= 48 && event.charCode <= 57' maxlength="3">
                                </div>
                            </div>
                            <div class="col-sm-12 form-group">
                                <button id="cmConfirm" type="button" class="btn btn-primary">Confirm <span
                                        class="glyphicon glyphicon-ok-sign"></span></button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade modal-fullscreen  " id="Meal_Update" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade modal-fullscreen  " id="Balance_Update" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" style="font-size: 50px;font-weight: lighter; color: #3fb0ac"
                        data-dismiss="modal" aria-hidden="true">X</button>
                </div>
                <div class="modal-body container-fluid ">
                    <div class="up_head">
    ...
    <div class="modal fade modal-fullscreen  " id="Extra_Cost_Update" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade modal-fullscreen  " id="mem_req" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade modal-fullscreen  " id="UpdateProfile" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <!-- This Model For My Meal-->

    <div class="modal fade modal-fullscreen  " id="MealStatus" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="container">
                        ...
                        <div class="row mdl_edit text-center">
                            ...
                            <div class="col-sm-12 form-group">
                                ...
                                <input class="form-control" type="text" id="MLamount" maxlength="4" readonly
                                    value="<?php echo $curMeal; ?>">
                            </div>
                            <div class="col-sm-12 form-group">
                                <div class="col-sm-12 form-group">
                                    <button id="Half" type="button" class="btn btn-info pull-left">Add Half <span
                                            class="glyphicon glyphicon-ok-sign"></span></button>
                                    <button id="Zero" type="button" class="btn btn-info">Set Zero <span
                                            class="glyphicon glyphicon-ok-sign"></span></button>
                                    <button id="Full" type="button" class="btn btn-info pull-right">Add Full <span
                                            class="glyphicon glyphicon-ok-sign"></span></button>
                                </div>
                            </div>
                            <div class="col-sm-12 form-group">
                                <button id="CHbtn" type="button" class="btn btn-primary">Save <span
                                        class="glyphicon glyphicon-ok"></span></button>
                            </div>
                        </div>
                    </div>
